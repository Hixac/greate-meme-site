...
class GMSError(Exception):
    """
    Base exception class for all errors raised by GMS.
...
class GMSRedirectionError(GMSError):
    """
...
class InternalServerError(GMSError):
    def __init__(
...
class ResourceNotFound(GMSError):
...
class UnprocessableContent(GMSError):
    def __init__(
        self,
        message: str,
        status_code: int = 422
...
class GMSRequestValidationError(GMSError):
